...
def process_message(
    ...
    try:
        ...
        error_msg = f"Database error in message handler: {str(e)}"
...
def process_whatsapp_message(
    ...
    try:
        ...
        error_msg = f"Database error in WhatsApp handler: {str(e)}"
        logger.error(error_msg)
        raise DatabaseError(
            error_msg,
            error_code=ErrorCode.DATABASE_ERROR,
...
def broadcast_message(
    ...
    try:
        ...
        if isinstance(result, dict):
            if "summary" in result and "_meta" not in result["summary"]:
        ...
        error_msg = f"Database error in broadcast handler: {str(e)}"
        logger.error(error_msg)
        raise DatabaseError(
            error_msg,
            error_code=ErrorCode.DATABASE_ERROR,
