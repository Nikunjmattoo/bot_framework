...
def transaction_scope(
    ...
    # Check for timeout during execution without event listener
    def check_timeout():
    ...
    try:
        ...
        if "deadlock" in str(e).lower() or "lock" in str(e).lower():
            raise DatabaseError(
                "Database deadlock or lock timeout occurred",
                error_code=ErrorCode.DATABASE_ERROR,
                original_exception=e,
                details={
                    "error_type": "deadlock_or_lock_timeout",
    ...
    while attempt <= max_retries:
        ...
        try:
            ...
            if attempt <= max_retries:
                ...
                # Create a new session if the current one is in a bad state
                if not db.is_active:
                    logger.warning("Session is no longer active, creating a new one")
