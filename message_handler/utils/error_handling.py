...
def with_error_handling(
    ...
    Example:
        @with_error_handling(
            ...
            expected_exceptions={ValueError: ErrorCode.VALIDATION_ERROR}
        )
        def process_message(db, content, instance_id, ...):
    ...
    def decorator(func: Callable[..., R]) -> Callable[..., R]:
        ...
        def wrapper(*args, **kwargs) -> R:
            ...
            try:
                ...
                # Re-raise exceptions in the reraise list without wrapping
                logger.warning(f"Re-raising exception {type(e).__name__}: {str(e)}")
                raise
                
            except ValidationError as e:
                # Special case for validation errors - add operation context but preserve the original error
                logger.warning(f"Validation error in {operation}: {str(e)}")
                # If we have a field attribute, update it with operation context
                if hasattr(e, 'details') and e.details is None:
                    e.details = {}
