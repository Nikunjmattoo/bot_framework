...
def broadcast_message_internal(
    ...
    try:
        ...
        if parallel and len(unique_user_ids) > 1:
            ...
            # Define worker function that creates its own DB session
            def process_user_with_own_session(user_id):
                """Process broadcast for one user with dedicated DB session."""
                ...
                with next(get_db()) as thread_db:
                    return process_broadcast_to_user(
                        thread_db, user_id, instance_id, content, user_trace_id
                    )
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                # Submit tasks
                futures = [
                    executor.submit(process_user_with_own_session, user_id)
