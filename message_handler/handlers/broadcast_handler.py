...
def validate_broadcast_parameters(
    ...
    if len(unique_ids) < len(user_ids):
        logger.warning("Duplicate user IDs found in broadcast list")
        # Convert to list of unique IDs (no error, just warning)
...
def process_broadcast_to_user(
    ...
    try:
        ...
        with retry_transaction(db, trace_id=trace_id, max_retries=2) as tx:
            ...
            logger.info(f"Broadcast message sent to user {user_id}: {message.id}")
            return {
                "user_id": user_id,
                "success": True,
...
def broadcast_message_internal(
    ...
    Returns:
        List of broadcast results for each user
