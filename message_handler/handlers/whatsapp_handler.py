...
SUPPORTED_MESSAGE_TYPES = ["text", "image", "audio", "document", "location", "contact"]

# Phone number regex for basic E.164 validation
...
def validate_whatsapp_message(
    ...
    if not PHONE_REGEX.match(from_number):
        logger.warning(f"Invalid 'from' phone number format: {from_number}")
        raise ValidationError(
            "WhatsApp 'from' number must be in E.164 format",
            field="from",
            value=from_number
        )
    ...
    if not PHONE_REGEX.match(to_number):
        ...
        raise ValidationError(
            "WhatsApp 'to' number must be in E.164 format",
            field="to",
            value=to_number
        )
...
def extract_whatsapp_data(
    ...
    wa_to = wa_metadata.get("to") or whatsapp_message.get("to")
    ...
    # Text message
    if "text" in whatsapp_message and "body" in whatsapp_message["text"]:
        message_type = "text"
        message_content = whatsapp_message["text"]["body"]
    
    # Image message
    elif "image" in whatsapp_message:
        message_type = "image"
        message_content = "[IMAGE]"
        if "caption" in whatsapp_message["image"]:
            message_content += f": {whatsapp_message['image']['caption']}"
        media_url = whatsapp_message.get("image", {}).get("url", None)
    
    # Audio message
    elif "audio" in whatsapp_message:
        message_type = "audio"
        message_content = "[AUDIO]"
        media_url = whatsapp_message.get("audio", {}).get("url", None)
    
    # Document message
    elif "document" in whatsapp_message:
        message_type = "document"
        doc_info = whatsapp_message["document"]
        message_content = f"[DOCUMENT: {doc_info.get('filename', 'unnamed')}]"
        if "caption" in doc_info:
            message_content += f": {doc_info['caption']}"
        media_url = doc_info.get("url", None)
    
    # Location message
    elif "location" in whatsapp_message:
    ...
    # Contact message
    elif "contact" in whatsapp_message:
        message_type = "contact"
        contacts = whatsapp_message.get("contacts", [])
        contact_names = []
        for contact in contacts:
            name_parts = []
            if "name" in contact:
                if contact["name"].get("formatted_name"):
                    name_parts.append(contact["name"]["formatted_name"])
                elif contact["name"].get("first_name"):
                    name_parts.append(contact["name"]["first_name"])
                    if contact["name"].get("last_name"):
                        name_parts.append(contact["name"]["last_name"])
...
def process_whatsapp_message_internal(
    ...
    try:
        ...
        # 2. Create unique idempotency key for WhatsApp
        ...
        with idempotency_lock(db, idempotency_key, trace_id) as lock_result:
            ...
            # 5. Process within a transaction 
            with transaction_scope(db, trace_id=trace_id) as tx:
                # Prepare WhatsApp user context
                user = prepare_whatsapp_user_context(tx, wa_data, instance_id, trace_id)
