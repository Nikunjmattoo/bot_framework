...
from typing import Dict, Any, Optional, Generator, Union, List
...
def _is_lock_orphaned(lock: IdempotencyLockModel) -> bool:
    """
    ...
    """
    if not lock or not lock.created_at:
        return True
    
    # Ensure lock.created_at is timezone-aware
    lock_time = lock.created_at
...
def _release_lock(db: Session, lock_id: Union[str, uuid.UUID], logger) -> bool:
    ...
    try:
        ...
        # If ORM delete failed, try SQL statement
        sql = text(f"DELETE FROM idempotency_locks WHERE id = :lock_id")
        db.execute(sql, {"lock_id": str(lock_id)})
        db.flush()
        
        logger.info(f"Released lock ID {lock_id} using SQL")
        return True
...
def idempotency_lock(
    ...
    try:
        ...
        if lock_id:
            try:
                # First try to release the lock in the current session
                released = _release_lock(db, lock_id, logger)
                
                # If that fails, try with a new connection
                if not released:
                    logger.warning(f"Failed to release lock in current session, trying with new connection")
                    with next(get_db()) as cleanup_db:
                        _release_lock(cleanup_db, lock_id, logger)
                        cleanup_db.commit()
            except Exception as e:
