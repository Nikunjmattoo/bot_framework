...
def _is_token_manager_available() -> bool:
    ...
    try:
        return importlib.util.find_spec("token_manager") is not None
...
def process_token_management(
    ...
    try:
        # Check if token manager module is available
        if not _is_token_manager_available():
            logger.warning("Token manager module not available, skipping token management")
            return False
        
        # Import here to avoid circular imports
        from token_manager.service import TokenService
...
def get_token_budgets(
    ...
    Returns:
        Token budgets dict or None if not available
    """
    logger = get_context_logger(
        "token_service", 
        trace_id=trace_id,
    ...
    try:
        # Check if token manager module is available
        if not _is_token_manager_available():
            logger.warning("Token manager module not available, returning default token budget")
            return {
                "default": {
        ...
        # Return a default budget as fallback
        return {
            "default": {
                "prompt_in": DEFAULT_TOKEN_BUDGET,
                "completion_out": DEFAULT_TOKEN_BUDGET,
...
def update_token_usage(
    ...
    try:
        ...
        # Check if token manager module is available
        if not _is_token_manager_available():
            logger.warning("Token manager module not available, skipping token usage update")
            return False
...
def check_token_budget(
    ...
    try:
        # Check if token manager module is available
        if not _is_token_manager_available():
            logger.warning("Token manager module not available, assuming budget is sufficient")
            return True
