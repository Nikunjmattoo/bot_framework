...
def validate_orchestrator_response(
        ...
        for field in ["llm_response", "message", "content", "response"]:
            if field in response and response[field]:
                if isinstance(response[field], str):
                    ...
                    response_text = response[field]["content"]
                    break
    ...
    if "timestamp" not in normalized_response:
        normalized_response["timestamp"] = datetime.now(timezone.utc).isoformat()
...
def extract_token_usage(
    ...
    if "token_usage" in orchestrator_response and isinstance(orchestrator_response["token_usage"], dict):
    ...
    elif "usage" in orchestrator_response and isinstance(orchestrator_response["usage"], dict):
    ...
    try:
        ...
        if token_usage:
            try:
                ...
                module_name = "compose"
                if "module" in orchestrator_response:
        ...
        error_msg = f"Database error in core processing: {str(e)}"
        logger.error(error_msg)
        raise DatabaseError(
            error_msg,
            error_code=ErrorCode.DATABASE_ERROR,
            original_exception=e,
            operation="process_core"
        )
    except Exception as e:
