...
# Orchestrator integration with environment-aware fallback
try:
    ...
    if ENVIRONMENT == "production":
        # Production: Fail fast, don't mask the problem
        ...
        # Development/Testing: Use mock with clear warning
        def process_orchestrator_message(adapter: Dict[str, Any]) -> Dict[str, Any]:
...
def process_core(
    ...
    try:
        ...
        # 5. Record token usage if available
        token_usage = {}
        if orchestrator_response and isinstance(orchestrator_response, dict):
        ...
        if token_usage:
            try:
                # Get template_key from orchestrator response or use default
                ...
                # Record token usage using TokenManager
