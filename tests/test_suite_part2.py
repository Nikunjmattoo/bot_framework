...
class TestAPIMessages:
    ...
    def test_send_message_invalid_instance(self, test_client):
        ...
        data = response.json()
        assert data["success"] is False
        assert "RESOURCE_NOT_FOUND" in str(data.get("error", {}))
...
class TestIdempotency:
    ...
    def test_idempotency_with_concurrent_requests(
        self,
        test_client,
        test_instance,
        idempotency_key,
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(test_client.post, "/api/messages", json=request_data)
                for _ in range(5)
            ]
        ...
        message_ids = [r.json()["data"]["message_id"] for r in responses]
        assert len(set(message_ids)) == 1
        
        # Orchestrator should only be called once (or handle concurrency)
        # May be called more than once due to race conditions, but should be minimal
        assert mock_orchestrator_success.call_count <= 2
