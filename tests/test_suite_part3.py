...
class TestExceptionHandling:
    ...
    def test_resource_not_found_error(self, test_client):
        ...
        data = response.json()
        assert data["success"] is False
        assert "RESOURCE_NOT_FOUND" in str(data["error"])
    ...
    def test_unexpected_exception_handling(self, test_client, test_instance):
        ...
        with patch('message_handler.core.processor.process_core') as mock:
    ...
    def test_trace_id_in_error_response(self, test_client, trace_id):
        ...
        data = response.json()
        # Trace ID should be in response or headers
        assert trace_id in str(response.headers) or trace_id in str(data)
...
class TestPerformance:
    ...
    def test_concurrent_message_processing(
        ...
        stats = performance_tracker.get_stats("concurrent_message")
        assert stats["success_rate"] == 1.0
        assert stats["avg"] < 5.0  # Average should be under 5 seconds
...
class TestTransactions:
    ...
    def test_retry_on_deadlock(self, test_db):
        """Test retry mechanism on deadlock"""
        ...
        with retry_transaction(test_db, max_retries=3) as tx:
            result = failing_operation()
        
        assert result == "success"
        assert attempt_count[0] == 3  # Should retry twice
...
class TestErrorRecovery:
    ...
    def test_recover_from_temporary_db_failure(self, test_db, test_instance):
        """Test system recovers from temporary database failure"""
        from message_handler.utils.transaction import retry_transaction
        ...
        def flaky_operation():
            ...
            # Create a message on second attempt
            message = MessageModel(
                id=uuid.uuid4(),
                session_id=uuid.uuid4(),
                user_id=uuid.uuid4(),
        ...
        with retry_transaction(test_db, max_retries=3):
...
class TestIntegrationScenarios:
    ...
    def test_broadcast_with_mixed_results(
        self,
        test_client,
        test_instance,
        test_db,
        ...
        """Test broadcast with some successes and failures"""
        ...
        user_ids = valid_users + [str(uuid.uuid4()), str(uuid.uuid4())]
        
        response = test_client.post("/api/broadcast", json={
            "instance_id": str(test_instance.id),
        ...
        # Should have results for all user IDs
        assert len(data["data"]["results"]) == len(user_ids)
        
        # Should have summary with success/failure counts
        assert "summary" in data["data"]
        summary = data["data"]["summary"]
        assert summary["total"] == len(user_ids)
        assert summary["successful"] >= 3  # At least the valid users
...
class TestStressAndLoad:
    ...
    def test_high_volume_messages(
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            results = list(executor.map(send_message, range(message_count)))
